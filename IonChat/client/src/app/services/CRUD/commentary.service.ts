...
export class CommentaryService {
   ...
   url = environment.api + 'commentary/';
   ...
   post(commentary: Commentary): Promise<any> {
      return this.http.post(this.url, JSON.stringify(commentary), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(commentary: Commentary): Promise<any> {
      return this.http.put(this.url, JSON.stringify(commentary), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
