...
export class InterestService {
   ...
   post(interest: Interest): Promise<any> {
      return this.http.post(this.url, JSON.stringify(interest), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(interest: Interest): Promise<any> {
      return this.http.put(this.url, JSON.stringify(interest), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
